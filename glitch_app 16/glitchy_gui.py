...
class LiveSetupPage(QWidget):
    ...
    def __init__(self, on_switch_mode, on_about):
        ...
        default_clip_dir = str(Path.home() / "Documents" / "Glitchy" / "glitch_clips")
        self.output_folder_edit.setPlaceholderText(f"Defaults to {default_clip_dir}")
        browse_btn = QPushButton("Browse\u2026")
        browse_btn.clicked.connect(self.browse_output_folder)
        row.addWidget(self.output_folder_edit, 1)
        row.addWidget(browse_btn)
...
class LiveRunningPage(QWidget):
    ...
    def __init__(self):
        ...
        self.status_dot.setStyleSheet(f"background: {COLORS['success']}; border-radius: 5px;")
        col = QVBoxLayout()
        title_row = QHBoxLayout()
        title_row.addWidget(self.status_dot)
        self.title_label = h2("Monitoring Live")
        ...
        top.addLayout(col)
        top.addStretch()
        self.open_folder_btn = QPushButton("Open Clips Folder")
        self.open_folder_btn.clicked.connect(self.open_clips_folder)
        self.open_folder_btn.setVisible(False)
        top.addWidget(self.open_folder_btn)
        self.action_btn = QPushButton("Stop"); self.action_btn.setProperty("role", "danger")
    ...
    def open_clips_folder(self):
        if self.clip_dir and Path(self.clip_dir).exists():
            QDesktopServices.openUrl(QUrl.fromLocalFile(str(self.clip_dir)))
        else:
            QMessageBox.information(self, "No clips folder",
                                     "No clips folder exists yet -- nothing was flagged, or clip saving was off.")

    def reset_for(self, source_desc, duration_seconds, clip_dir=None):
        self._running = True
        self.status_dot.setStyleSheet(f"background: {COLORS['success']}; border-radius: 5px;")
        self.title_label.setText("Monitoring Live")
        ...
        self.action_btn.setProperty("role", "danger")
    ...
    def set_stopped(self, reason):
        self._running = False
        self.status_dot.setStyleSheet(f"background: {COLORS['text_ghost']}; border-radius: 5px;")
        self.title_label.setText("Monitoring Stopped")
        self.action_btn.setText("Back to Setup")
        self.action_btn.setProperty("role", "")
        self.action_btn.style().unpolish(self.action_btn)
        self.action_btn.style().polish(self.action_btn)
        if self.clip_dir:
            self.open_folder_btn.setVisible(True)
            self.footer_label.setText(f"{reason} \u2014 {self.alert_count} alert"
                                       f"{'s' if self.alert_count != 1 else ''} this session. "
                                       f"Clips saved to: {self.clip_dir}")
        else:
            self.footer_label.setText(f"{reason} \u2014 {self.alert_count} alert"
                                       f"{'s' if self.alert_count != 1 else ''} this session.")
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.live_running_page.done_requested.connect(self._live_session_done)
    ...
    def start_live_monitor(self):
        ...
        if settings["extract_clips"] and not settings["clip_dir"]:
            # Path.cwd() for a double-clicked/Finder-launched app resolves
            # to something the user has no way to predict or find (often
            # "/" or the app bundle's own directory) -- Documents is always
            # writable and always somewhere the user can actually locate.
            settings["clip_dir"] = str(Path.home() / "Documents" / "Glitchy" / "glitch_clips")
        ...
        self.live_clip_dir = settings["clip_dir"] if settings["extract_clips"] else None
        self.live_running_page.reset_for(source_desc, self.live_duration, self.live_clip_dir)
    ...
    def _poll_live(self):
        ...
        if self.live_duration and elapsed >= self.live_duration:
            self.stop_live_monitor(reason="Stopped automatically \u2014 timer elapsed")

    def stop_live_monitor(self, reason="Stopped"):
        ...
        self.live_source = None
        self.live_detector = None
        # Stay on this page showing the final log/stats/clip location --
        # rather than immediately bouncing back to setup, which is exactly
        # why the alert log and "where did my clips go" were invisible
        # right when you'd actually want to check them.
        self.live_running_page.set_stopped(reason)

    def _live_session_done(self):
